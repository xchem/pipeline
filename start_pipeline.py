...
from luigi_classes.prepare_fragalysis import BatchCreateSymbolicLinks, BatchAlignTargets, BatchCutMaps, BatchCreateBiomoledPDB
...
sentry_string = str("https://" + SentryConfig().key +
                    "@sentry.io/" + SentryConfig().ident)
# initiate sentry sdk
...
class PostPipeClean(luigi.Task):
    ...
    def run(self):
        ...
        paths = [x for x in glob.glob(os.path.join(
            self.log_directory, '*', '*')) if 'done' in x]
        ...
        paths = [x for x in paths if 'cut' not in x]
