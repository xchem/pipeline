...
def get_update_times(request):
    ...
    data = [{'file': f, 'db_date': datetime.datetime.strptime(str(dbd), '%Y%m%d%H').strftime('%Y-%m-%dT%H'), 'rt_date': datetime.datetime.strptime(str(rtd), '%Y%m%d%H%M%S').strftime('%Y-%m-%dT%H')}
