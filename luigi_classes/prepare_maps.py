...
class CutOutEvent(luigi.Task):
    ...
    def run(self):
        ...
        obConv.WriteFile(mol, self.mol_file.replace('.sdf', '.pdb'))

        mapmask = '''module load ccp4 && mapmask mapin %s mapout %s xyzin %s << eof
            border %s
            end
        eof
        ''' % (self.mapin, self.mapout, self.mol_file.replace('.sdf', '.pdb'), str(self.border))

        process = subprocess.Popen(str(self.ssh_command + ' "' + 'cd ' + self.directory + ';' + mapmask + '"'),
                                   shell=True, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        out, err = process.communicate()

        os.remove(os.path.join(self.directory, self.mol_file.replace('.sdf', '.pdb')))
